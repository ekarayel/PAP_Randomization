...
next
  ...
  also have "... = map_pmf time p \<bind> (\<lambda>x. replicate_pmf n (map_pmf time p) \<bind> 
    (\<lambda>y. return_pmf (x + sum_list y)))"
    by (subst Suc) (metis (no_types, lifting) bind_map_pmf bind_pmf_cong)
  also have "... = map_pmf sum_list (replicate_pmf (Suc n) (map_pmf time p))"
...
proof -
  ...
  also have "... = bind_pmf ?m (\<lambda>x. return_pmf (time x + time (min_list_tm (val x))))"
    unfolding lift_tm_def bind_return_pmf by simp
  also have "... = bind_pmf ?m (\<lambda>x. return_pmf (time x + length (val x)))"
  ...
  also have "... = bind_pmf ?m (\<lambda>x. return_pmf (time x + length ps))"
    using set_val_replicate_tpmf(1)
    by (intro bind_pmf_cong refl arg_cong[where f="return_pmf"] arg_cong2[where f="(+)"]) auto
  also have "... = map_pmf (\<lambda>x. x + length ps) (map_pmf time ?m)"
...
proof -
  ...
  also have "... \<le> 5+6*length s + 2*length t + (1+length t)" using a by simp
  also have "... = 36 + 3*length t" unfolding s_def by simp
  also have "... \<le> 36 + 3 * (1+length (remove1 p t))" 
...
proof -
  ...
  also have "... \<le> 2* (length (val (lookup_neighborhood ?g p))) +1 +
...
proof -
  ...
  also have "... = (\<Sum>x\<leftarrow>ps. size (mset (val (lookup_neighborhood (grid ps d) x))))"
    by simp
  also have "... \<le>
  ...
  also have "... \<le> (\<Sum>x\<leftarrow>ps. size({#y\<in># mset ps. \<forall>k\<in>{1,2}. \<bar>\<lfloor>y$k/d\<rfloor>-\<lfloor>x$k/d\<rfloor>\<bar>\<le>1 #}))"
    unfolding to_grid_def by (intro sum_list_mono size_mset_mono multiset_filter_mono_2) auto
  also have "... \<le> (\<Sum>x\<leftarrow>ps. size({#y\<in># mset ps. dist y x < d * real_of_int (1 + 1) * sqrt 2#}))"
    using exhaust_2
    by (intro sum_list_mono size_mset_mono multiset_filter_mono_2 grid_dist_upperI[OF assms(1)])
      blast
  also have "... = (\<Sum>x\<leftarrow>ps. length (filter (\<lambda>y. dist x y < 2 * sqrt 2 * d) ps))"
  ...
  also have "... \<le> 2 + 4*length ps + (length s + time (min_list_tm s)) + 
  ...
  also have "... \<le> 2 + 44* length ps + 7 * close_point_size ps (2* sqrt 2*d)"
...
proof -
  ...
  have "ennreal ?L = (\<integral>ds. real (close_point_size ps (min_list ds)) \<partial>replicate_pmf ?n ?dpmf)"
    unfolding a by simp
  also have "... \<le> (\<integral>ds. real (?n + 2*?cps (min_list ds)) \<partial>replicate_pmf ?n ?dpmf)" using T_fin_ne
  ...
  also have "... = ennreal ?n + 2*ennreal(\<integral>ds. real (?cps (min_list ds)) \<partial>replicate_pmf ?n ?dpmf)"
    by (simp add:ennreal_mult' integrable_measure_pmf_finite finite_replicate_pmf T_fin_ne)
  also have "... = ennreal ?n +2*\<integral>\<^sup>+ x. ennreal (real (?cps (min_list x))) \<partial>replicate_pmf ?n ?dpmf"
  ...
  also have "... = ennreal ?n +2*\<integral>\<^sup>+ x. ennreal_of_enat (?cps (min_list x)) \<partial>replicate_pmf ?n ?dpmf"
  ...
  also have "... = ennreal ((real ?n*real (?n+1) + ?n * (?n-1)) / real (?n+1)) "
    by (intro arg_cong[where f="ennreal"]) (simp add:field_simps)
  also have "... = ennreal (2*real ?n * real ?n / real (?n+1)) "
    using assms by (simp add:field_simps)
  also have "... \<le> ennreal (2*real ?n * real ?n / real ?n) "
